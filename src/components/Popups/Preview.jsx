  ...
  return <div class="compact side card bg-pink-200 hover:bg-gradient-to-r from-pink-500 from-10% via-emerald-500 via-30% to-blue-500 to-90% shadow-xl hover:text-white">
            <div class="absolute inline-flex right-0 p-2 gap-1 font-bold ">
              <div onClick={()=>{
                ...
                navigate('/contact/contactEdit')
                onEditform(id)}}  class="inline text-xl  hover:text-green-500  cursor-pointer text-black">
               ...
               <Link to={`/contac/${id}`}>
                  <DeletIcon onClick={() => setViewContacts(false)} class="align-middle text-xl hover:text-red-600 text-black cursor-pointer" />
               </Link> 
            </div>
            <button onClick={() => popupProfile(id)}>
              <div class="card-body flex-row items-center space-x-4">
                ...
                <div>
                  <h2 className="card-title text-black">{first_name} {last_name}</h2>
                  <div className="text-base-content font-bold hover:text-white text-black">
                  {mobile_no}
                  </div>
                </div>
              </div>
